...
public class RiskManager {
    ...
    public void monitor() {
        marginRateManage(null);
        positionRiskManage(null);
        profitLossRiskManage(null);
    }
    ...
    public void marginRateManage(String contractCode) {
        //获取该合约账户的对应币种的保证金率
        BigDecimal marginRate = riskContext.getRiskRate();
        // todo 通过symbol加contractType去取
        ...
        if (marginRate.compareTo(marginRateLimit3) < 0) {
            ...
            //正常情况，当摆单状态为停止下开仓单，只下平仓单的话，将摆盘状态设为正常运行
    ...
    private void profitLossMethodA(BigDecimal onceProfitLoss, String contractCode) {
        logger.warn("警报：本次盈利亏损大于阈值:{},[contractCode={}]", onceProfitLoss, contractCode);
    }
