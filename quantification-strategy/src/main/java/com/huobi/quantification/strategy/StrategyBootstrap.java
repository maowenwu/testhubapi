...
public class StrategyBootstrap {
    ...
    public void startInstance(StrategyInstanceConfig config) {
        ...
        logger.info("注册job开始");
        // percent10
        jobManageService.addHuobiSpotDepthJob(getSymbol(config.getSpotBaseCoin(), config.getSpotQuotCoin()), config.getSpotDepthType(), "0/1 * * * * ?", true);
        ...
        jobManageService.addHuobiFutureDepthJob(getSymbol(config.getFutureBaseCoin(), config.getFutureQuotCoin()), "this_week", config.getFutureDepthType(), "0/1 * * * * ?", true);
        jobManageService.addHuobiFutureDepthJob(getSymbol(config.getFutureBaseCoin(), config.getFutureQuotCoin()), "next_week", config.getFutureDepthType(), "0/1 * * * * ?", true);
        jobManageService.addHuobiFutureDepthJob(getSymbol(config.getFutureBaseCoin(), config.getFutureQuotCoin()), "quarter", config.getFutureDepthType(), "0/1 * * * * ?", true);
        logger.info("注册job完成");
        // 等待3秒，保证job已经完全运行
    ...
    public void stopInstance() {
        ...
        // percent10
        jobManageService.addHuobiSpotDepthJob(getSymbol(config.getSpotBaseCoin(), config.getSpotQuotCoin()), config.getSpotDepthType(), "0/1 * * * * ?", false);
        ...
        jobManageService.addHuobiFutureDepthJob(getSymbol(config.getFutureBaseCoin(), config.getFutureQuotCoin()), "this_week", config.getFutureDepthType(), "0/1 * * * * ?", false);
        jobManageService.addHuobiFutureDepthJob(getSymbol(config.getFutureBaseCoin(), config.getFutureQuotCoin()), "next_week", config.getFutureDepthType(), "0/1 * * * * ?", false);
        jobManageService.addHuobiFutureDepthJob(getSymbol(config.getFutureBaseCoin(), config.getFutureQuotCoin()), "quarter", config.getFutureDepthType(), "0/1 * * * * ?", false);
