...
public class MarketService {
	...
	public Map<String, BigDecimal> getHuoBiSpotBuyOneSellOnePrice(SpotDepthReqDto spotDepthReqDto) {
		...
		BigDecimal buyPrice=serviceResult.getData().getData().getAsks().get(0).getPrice();//买盘,[price(成交价), amount(成交量)], 按price降序,
		BigDecimal sellPrice=serviceResult.getData().getData().getBids().get(0).getPrice();//卖盘,[price(成交价), amount(成交量)], 按price升序
