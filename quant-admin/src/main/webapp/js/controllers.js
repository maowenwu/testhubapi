 ...
 *
 */
function MainCtrl($scope,notify,$http) {
	/**
	 * 常量
	 */
	$scope.principal = _principal;
	$scope.csrfData = $("meta[name=_csrf]").attr('content');
	
	$scope.permits = _permits;
	$scope.hasPermit = function(permit){
		return $scope.permits.indexOf(permit) >= 0;
	};
	$scope.closePage = function(){
		window.close();
	}
	$scope.logout=function(){
		$http.post('logout.do',_parameterName+"="+_token,{headers:{'Content-Type': 'application/x-www-form-urlencoded'}})
		.success(function(req){  
			window.location.href="login.do?logout";
		}).error(function(){
		});
	}
	 ...
	 * 用户修改密码
	 */
	$scope.userUpPwdOpen=function(){
		$scope.userInfo={};
		$('#updatePwd').modal('show');
	}
	$scope.submitNewUserPwd=function(){
		if($scope.userInfo.newPwd!=$scope.userInfo.newPwds){
			$scope.notice('新密码不一致');
			return;
		}
		var data={"pwd":$scope.userInfo.password,"newPwd":$scope.userInfo.newPwd};
		$http.post('user/updatePwd.do',angular.toJson(data))
		.success(function(req){  
			if(req.bols){
	...
	$scope.boolAll=[{text:"全部",value:""},{text:"是",value:1},{text:"否",value:0}];
	$scope.checkType=[{text:"已审核",value:1},{text:"未审核",value:0}];
	$scope.status=[{text:"有效",value:1},{text:"无效",value:0}];
	$scope.statusAll=[{text:"全部",value:2},{text:"开启",value:1},{text:"关闭",value:0}];
	 ...
	 * 显示毫秒数 duration:
	 */
	$scope.notice=function(message,options){
		var option={ 
			message: '测试提示！',
			classes: 'alert-info', 
		...
		o=angular.extend({}, option, options);
		o.message=message?message : o.message;
		notify(o);
	};
	 ...
	 * 
	 */
//	$scope.confirm=function(option,callback){
//		var _option={
//		title: "修改服务状态?",
...
//	        function (isConfirm) {
//	            if (isConfirm) {
//	                $http.post("service/updateServiceStatus",{serviceId:id,status:_statuts})
...
//    .controller('jstreeCtrl', jstreeCtrl)
    .filter("formatDropping",function(){
	    return function(input,params){
	    	for(var i in params){
	    		if(input==params[i].value){
    ...
    .filter('serviceTypeFilter', function () {
	    var serviceType = initData.SERVICE_TYPE || {};
	    var keyValue = {};
	    for (var index in serviceType){
	        keyValue[serviceType[index].value] = serviceType[index].text;
	    }
	    return function (value) {
 ...
 * mays
 * 2018-02-07
 * type:起始时间调用传1，结束时间调用传2
 * st:	起始时间id
 * et:	结束时间id
 * day:	限制天数
 * isVerifyTime: 是否校验时间，校验：1，不校验：0
 */
verifyTime=function(type,st,et,day,isVerifyTime){
	if (!isVerifyTime) {
		return;
	}
...
//			$scope.info.planTimeEnd = moment(setTime).format("YYYY-MM-DD HH:mm:ss");
			setEndTime(setTime);
			$(et).val(moment(setTime).format("YYYY-MM-DD HH:mm:ss"));
		}
	} else { // 填完结束时间
